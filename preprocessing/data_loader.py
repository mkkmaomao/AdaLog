...
def bert_encoder(s, no_wordpiece=0):
    ...
    # v = tf.reduce_mean(outputs.last_hidden_state, 1)
    v = tf.reduce_mean(last_hidden_states, 1)
    # print("v[0]:",v[0])
    return v[0]
...
def load_HDFS(log_file, label_file=None, train_ratio=0.8, window='session',
              split_type='uniform',
              save_csv=True,
              window_size=0,
              e_type="bert"):
    ...
    if log_file.endswith('.npz'):
        ...
        if save_csv:
            data_df.to_csv('data_instances.csv', index=True)
...
def load_Supercomputers(log_file, train_ratio=0.8, windows_size=100, step_size=0, e_type='bert', e_name=None,
    ...
    # with open(log_file, mode="rb") as f:
        ...
        # ------------- original ---------------
        logs = f.readlines()
        logs = [x.decode().strip() for x in logs]
        # logs = [x.strip() for x in logs] for all datasets except spirit_5m
...
if __name__ == '__main__':
    ...
    (x_tr, y_tr), (x_te, y_te) = load_Supercomputers(
        "./data/raw/BGL.log", train_ratio=0.8, windows_size=100,
        step_size=0, e_type='bert', e_name=None, mode='imbalance')

    # with open("data/embeddings/Spirit_5m/iforest-train-ws100.pkl", mode="wb") as f:
    #     pickle.dump((x_tr, y_tr), f, protocol=pickle.HIGHEST_PROTOCOL)
    #
    # with open("data/embeddings/Spirit_5m/iforest-test-ws100.pkl", mode="wb") as f:
    #     pickle.dump((x_te, y_te), f, protocol=pickle.HIGHEST_PROTOCOL)
    # utf8 is for spirit_5m, others use "wb" and "rb"
    ...
    # (x_tr, y_tr), (x_te, y_te) = load_HDFS(
    #      "./HDFS-bert.npz", "./data/raw/HDFS/anomaly_label.csv", train_ratio=0.8, split_type='sequential')
    # with open("./data/embeddings/HDFS/neural-train.pkl", mode="wb") as f:
    #     pickle.dump((x_tr, y_tr), f, protocol=pickle.HIGHEST_PROTOCOL)
    #
    # with open("./data/embeddings/HDFS/neural-test.pkl", mode="wb") as f:
    #     pickle.dump((x_te, y_te), f, protocol=pickle.HIGHEST_PROTOCOL)

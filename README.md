...
*** 
**NOTE:**
In order to quickly implement AdaLog, we use the BGL dataset as an example. 

1. By executing Steps 1-3, the generated files can be found in the *data/embedding/BGL* folder. It contains the word embeddings of the training set with/without undersampling and the test set. The parameter *p* or *per* is defined as the undersampling ratio of normal and abnormal logs. For BGL, the values are 8 (ws=20), 7 (ws=100), and 6 (ws=200). 

2. The *k* values and the label probability are calculated, which can be obtained in the *clusters/BGL* folder. We provide the pickle file (BGL with ws=200) for loading label probabilities: [label probability](https://figshare.com/articles/dataset/Probability_BGL_ws_200/25523665)
